...
- New `timeResults` table: `studentId`, `date` (YYYY-MM-DD), `distanceMeters`
  (one of 50, 100, 200, 400, 800, 1500), `stroke` (skill key, validated
...
- `attendance`: current season-to-date attendance % ÷ target %, clamped
...
Monday 06:00 cron (`crons.ts`) runs an internal action that computes, per
...
`by_user_and_read`). Generated by: flag creation events (M8 rules
evaluated on a daily cron that also feeds S2), practice reminders
